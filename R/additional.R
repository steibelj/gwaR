...
qqgplot = function(pvector, add = F, ...) {
    ...
    if (!add) {
        plot(e, o, ..., xlab = expression(Expected ~ ~-log[10](italic(p))), ylab = expression(Observed ~ ~-log[10](italic(p))), xlim = c(0, 
            max(e)), ylim = c(0, max(o) + 1))
    } else {
        points(e, o, ...)
    }
    lines(e, e, col = "red")
...
plot.GWAS <- function(gwas, correction = "bonf", gpdata = NULL, plotlog10 = FALSE, pvalue = 0.05, q.qplot = FALSE, ...) {
    ...
    if (class(gpdata) != "gpData") 
        stop("object must be of class gpData")
    if (is.null(gpdata$map)) 
        stop("missing map in the gpData object")
    
    # Take out the NA of the map
    map <- na.omit(gpdata$map)
    idx <- rownames(map) %in% rownames(gwas)
    map <- map[idx, ]
    
    # Filter the SNPS as they appear in the map
    
    # Get the standarized values of the SNP
    snpej <- gwas[["ghat"]]/sqrt(gwas[["var_ghat"]])
    names(snpej) <- rownames(gwas)
    # Calculate the p value
    pval <- 2 * (1 - pnorm(abs(snpej)))
