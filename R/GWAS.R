...
summary.gblup <- function(gblup, ehat = FALSE, sigma = FALSE) {
    if (class(gblup) != "gblup") 
        stop("input must be a gblup object")
    ...
    if (sigma) 
        result$sigma <- c(gblup$sigma, h2 = gblup$sigma["G"]/sum(gblup$sigma))
    if (ehat) 
